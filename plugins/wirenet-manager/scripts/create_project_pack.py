...
def main() -> int:
    ...
    collections = [
        value
        for value in (bindings.get("bindings", []), bindings.get("routes", []))
        if isinstance(value, list)
    ]
    existing_local_paths = {
        row.get("path") for collection in collections for row in collection if isinstance(row, dict)
    }
    ...
    binding_rows = bindings.setdefault("bindings", [])
