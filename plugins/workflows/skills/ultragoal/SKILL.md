...
2. If the current workspace is outside Manager, use
   `$manager` to resolve an existing Project Pack when manager@wirenet is
   available. If it is untracked, let `$manager` classify it and do not activate
...
- On completion, distill the durable outcome into `RESULT.md` and reconcile the
  Project Pack through `$manager` when future work would otherwise
