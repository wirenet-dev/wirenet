...
This is the shared content contract for the wirenet Manager core. It
helps the agent maintain the Manager without requiring the user to understand
its folders.

## Workflow Ownership

- `$manager` uses this contract for orientation and durable Manager writes.
- `project-lifecycle.md` governs Project and Experiment classification,
  creation, bindings, and lifecycle.
- `person-context.md` governs canonical collaborator context.
- `workspace-sync.md` governs durable handoffs from tracked external workspaces.
- `$manager-setup` materializes the matching shelves and installs the minimal
  managed global `AGENTS.md` trigger after approval.

Keep this shared reference instead of creating a separate routing skill. The
managed global block supplies reliable recall; the Manager holds the procedure;
