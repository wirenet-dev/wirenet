...
2. `$manager` classifies an untracked workspace once through its lifecycle
   playbook.
3. `$manager` reconciles an already tracked workspace with the
   smallest durable diff.
4. A long-running `$manager` task reconciles TODOs, packets, people,
   and signals.
