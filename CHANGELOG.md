...
- Reduced generated Project and Experiment Pack frontmatter to one field per
  durable meaning. Removed unused scope aliases, per-concept `okf_profile`, and
  the duplicate `name`/`summary` vocabulary; standard OKF `title` and
  `description` now serve indexes, search, and display consistently.
- Added `scripts/tidy_frontmatter.py`, a dry-run-first, checkpoint-gated,
  Doctor-validated cleanup for existing packets. It preserves prose and unknown
  producer metadata while normalizing legacy routing and timestamp fields.
