...
description: Create or repair the user's wirenet Manager vault. Use when the user wants to set up their Manager for the first time, says "onboard me", asks to install or initialize the vault, wants to adopt an existing folder as their Manager, or when $manager found the vault missing or structurally broken. Runs once per machine; ongoing work and maintenance belong to $manager.
---

# manager-onboarding

Build the user's Manager through a conversation, not a configuration. The user
should end onboarding with a working vault and no learned structure words.

## Preflight

1. Resolve the target from `WIRENET_MANAGER_DIR`, then `~/Manager`.
2. If a healthy vault already exists, say so and hand off to `$manager` — do
...
Follow the vault shape in the `$manager` reference `vault-model.md`. Create
only the core files; optional folders appear later on first real content.

## Voice And Instructions

- Personalize the vault's root `AGENTS.md` from the interview: confirmed
...
- Offer the write-like-me bootstrap (see `references/write-like-me.md`): with
  ...
  personal write-like-me skill at the global skill root, outside the vault.
...
1. Initialize git in the vault and make the first commit. Offer a private
...
3. Register the vault as a qmd collection when qmd is available; skip quietly
   when it is not.
