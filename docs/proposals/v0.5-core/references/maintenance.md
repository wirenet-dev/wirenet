...
Ongoing health, plugin updates, and retrieval upkeep. This replaces a separate
setup/maintenance skill: onboarding runs once via `$manager-onboarding`;
everything recurring lives here.
...
- Fix findings as previewed proposals, smallest diff first. If the vault is
  ...
  `$manager-onboarding` in repair mode instead of improvising structure.
